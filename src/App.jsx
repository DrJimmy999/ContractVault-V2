      ...
      <div className="login-card">
        ...
        {sent ? (
          <div className="login-success">
            ...
            <span style={{ fontSize:12, marginTop:4, display:'block' }}>Check your inbox and click the link to continue.</span>
          </div>
        ) : (
          <form onSubmit={handleSend}>
            <div className="login-input-wrap">
              <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                placeholder="you@dubicars.com" required style={{ flex:1 }} />
        ...
        <div className="login-note">Access is restricted to @dubicars.com accounts set up by an admin.</div>
      </div>
    </div>
  )
}
        ...
        <nav className="nav">
          ...
          <div className="nav-right">
            ...
            <button className="btn btn-sm"
              style={{ background:'rgba(255,255,255,0.12)', color:'#fff', border:'0.5px solid rgba(255,255,255,0.2)', fontSize:12 }}
...
export default function App() {
  ...
  async function loadProfile(user) {
    setLoading(true)
    const { data, error } = await supabase.from('cv_users').select('*').eq('email', user.email).single()
    if (error || !data) {
  ...
  if (!profile) return <LoginPage authError={authError} />
